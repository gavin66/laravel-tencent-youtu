...
class YouTu
{
    ...
    public static function facecompare( $image_path_a, $image_path_b )
    {
        ...
        if ( !file_exists($real_image_path_a) ) {
            return [ 'httpcode' => 0, 'code' => self::HTTP_BAD_REQUEST, 'message' => 'file ' . $image_path_a . ' not exists', 'data' => [] ];
        }

        if ( !file_exists($real_image_path_b) ) {
            return [ 'httpcode' => 0, 'code' => self::HTTP_BAD_REQUEST, 'message' => 'file ' . $image_path_b . ' not exists', 'data' => [] ];
        }
    ...
    public static function addface( $person_id, array $image_path_arr, $facetag = '' )
    {
        ...
        foreach ( $image_path_arr as $one_path ) {
            if ( !file_exists($one_path) ) {
                return [ 'httpcode' => 0, 'code' => self::HTTP_BAD_REQUEST, 'message' => 'file ' . $one_path . ' not exists', 'data' => [] ];
            }
    ...
    public static function livedetectfour( $validate_data, $video_path, $card_path, $compare_card = false, $seq = '' )
    {
        ...
        if ( !file_exists($real_video_path) ) {
            return [ 'httpcode' => 0, 'code' => self::HTTP_BAD_REQUEST, 'message' => 'file ' . $video_path . ' not exists', 'data' => [] ];
        }
        ...
        if ( $compare_card ) {
            ...
            if ( !file_exists($real_card_path) ) {
                return [ 'httpcode' => 0, 'code' => self::HTTP_BAD_REQUEST, 'message' => 'file ' . $card_path . ' not exists', 'data' => [] ];
            }
    ...
    public static function pitu( $image_base64 )
    {
        $expired = time() + self::EXPIRED_SECONDS;
